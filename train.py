...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    # Start training
    t0 = time.time()
    nw = max(round(hyp['warmup_epochs'] * nb), 100) ##this doesnt work for my laptop
    nw = 4  # number of warmup iterations, max(3 epochs, 100 iterations)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with torch.cuda.amp.autocast(amp):
                ...
                loss, loss_items = compute_loss(pred, targets.to(device),imgs[i])  # loss scaled by batch_size
                if RANK != -1:
